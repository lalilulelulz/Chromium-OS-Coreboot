...
#define WP_GPIO		33
#define RECOVERY_GPIO	22
#define DEVMODE_GPIO	54
#define FORCE_RECOVERY_MODE	0
#define FORCE_DEVELOPER_MODE	1
...
void fill_lb_gpios(struct lb_gpios *gpios)
{
	...
	u16 gen_pmcon_1 = pci_read_config16(dev, GEN_PMCON_1);
	...
	/* Write Protect: GPIO active high */
...
int get_developer_mode_switch(void)
{
	...
	printk(BIOS_DEBUG,"DEVELOPER MODE FROM GPIO 0x%x: %x\n",DEVMODE_GPIO,
								 dev_mode);
...
int get_recovery_mode_switch(void)
{
	...
	/* Servo GPIO is active low, reverse it for intial state (request) */
	rec_mode = !get_pch_gpio(RECOVERY_GPIO);
	printk(BIOS_DEBUG,"RECOVERY MODE FROM GPIO 0x%x: %x\n",RECOVERY_GPIO,
								 ec_rec_mode);

#ifndef __PRE_RAM__
	ec_rec_mode = (ec_mem_read(EC_HW_GPI_STATUS) >>
						EC_GPI_RECOVERY_MODE_BIT);
	printk(BIOS_DEBUG,"RECOVERY MODE FROM EC: %x\n", ec_rec_mode);
#endif
	rec_mode |= ec_rec_mode;
	return rec_mode;
}
