...
static int get_mac_address (u32 *high_dword, u32 *low_dword)
{
	...
	int key_length = 12; 		/* length of 'ethernet_mac' */
	...
	offset = search (key, (char *)search_address, key_length, search_length);
	if (offset == search_length){
		printk(BIOS_DEBUG, "Error: Could not locate '%s' in VPD\n",key);
		return 0;
	}
	printk(BIOS_DEBUG, "Located '%s' in VPD at offset 0x%04x (0x%08lx)\n",
						key, offset, search_address);

	offset += key_length + 1; /* move past key and next character */
	*high_dword = 0;
...
static void program_mac_address (device_t dev)
{
	u16 io_base;
