...
static int stout_smbios_type41(int *handle, unsigned long *current,
...
static int stout_onboard_smbios_data(device_t dev, int *handle,
				     unsigned long *current)
{
	int len = 0;

	len += stout_smbios_type41(handle, current,
				  STOUT_LIGHTSENSOR_NAME,
				  STOUT_LIGHTSENSOR_IRQ,
				  STOUT_LIGHTSENSOR_I2C_ADDR);

	len += stout_smbios_type41(handle, current,
				  STOUT_TRACKPAD_NAME,
				  STOUT_TRACKPAD_IRQ,
				  STOUT_TRACKPAD_I2C_ADDR);

	len += stout_smbios_type41(handle, current,
				  STOUT_TOUCHSCREEN_NAME,
				  STOUT_TOUCHSCREEN_IRQ,
				  STOUT_TOUCHSCREEN_I2C_ADDR);
...
struct chip_operations mainboard_ops = {
	...
	.get_smbios_data = stout_onboard_smbios_data,
};
