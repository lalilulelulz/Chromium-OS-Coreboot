...
void early_xhci_disable_compliance_mode(void)
{
	...
	xhci_base = pci_read_config32(dev, PCI_BASE_ADDRESS_0) & ~0xf;
	if (!xhci_base) {
		xhci_base = EARLY_XHCI_BAR;
		pci_write_config32(dev, PCI_BASE_ADDRESS_0, xhci_base);
	}
