...
static void hdmi_set_clock_regenerator(u32 n, u32 cts)
{
	...
	writel(cts3, &hdmi_regs->aud_cts3);
	writel((cts >> 8) & 0xff, &hdmi_regs->aud_cts2);
	writel(cts & 0xff, &hdmi_regs->aud_cts1);

	/* write n values; n1 must be written last */
	n3 |= (n >> 16) & HDMI_AUD_N3_AUDN19_16_MASK;
	writel(n3, &hdmi_regs->aud_n3);
	writel((n >> 8) & 0xff, &hdmi_regs->aud_n2);
	writel(n & 0xff, &hdmi_regs->aud_n1);

	writel(HDMI_AUD_INPUTCLKFS_128, &hdmi_regs->aud_inputclkfs);
}
...
static void hdmi_video_sample(void)
{
	...
	writel(val, &hdmi_regs->tx_invid0);

	/* enable tx stuffing: when de is inactive, fix the output data to 0 */
	val = HDMI_TX_INSTUFFING_BDBDATA_STUFFING_ENABLE |
	      HDMI_TX_INSTUFFING_RCRDATA_STUFFING_ENABLE |
	      HDMI_TX_INSTUFFING_GYDATA_STUFFING_ENABLE;
	writel(val, &hdmi_regs->tx_instuffing);
	writel(0x0, &hdmi_regs->tx_gydata0);
	writel(0x0, &hdmi_regs->tx_gydata1);
	writel(0x0, &hdmi_regs->tx_rcrdata0);
	writel(0x0, &hdmi_regs->tx_rcrdata1);
	writel(0x0, &hdmi_regs->tx_bcbdata0);
	writel(0x0, &hdmi_regs->tx_bcbdata1);
}
...
static void hdmi_update_csc_coeffs(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(csc_coeff_default); i++) {
		for (j = 0; j < ARRAY_SIZE(csc_coeff_default[0]); j++) {
			...
			writel(coeff >> 8, &hdmi_regs->csc_coef[i][j].msb);
			writel(coeff && 0xff, &hdmi_regs->csc_coef[i][j].lsb);
		}
	}
...
static void hdmi_video_csc(void)
{
	...
	writel(interpolation, &hdmi_regs->csc_cfg);
...
static void hdmi_video_packetize(void)
{
	...
	writel(remap_size, &hdmi_regs->vp_remap);
...
static int hdmi_phy_wait_i2c_done(u32 msec)
{
	...
	do {
		val = readl(&hdmi_regs->ih_i2cmphy_stat0);
		if (val & 0x3) {
			writel(val, &hdmi_regs->ih_i2cmphy_stat0);
			return 0;
		}
...
{
	writel(0xff, &hdmi_regs->ih_i2cmphy_stat0);
	writel(addr, &hdmi_regs->phy_i2cm_address_addr);
	writel((u8)(data >> 8), &hdmi_regs->phy_i2cm_datao_1_addr);
	writel((u8)(data >> 0), &hdmi_regs->phy_i2cm_datao_0_addr);
	writel(HDMI_PHY_I2CM_OPERATION_ADDR_WRITE,
		&hdmi_regs->phy_i2cm_operation_addr);
...
static int hdmi_phy_configure(u32 mpixelclock)
{
	...
	/* phy reset */
	writel(HDMI_MC_PHYRSTZ_DEASSERT, &hdmi_regs->mc_phyrstz);
	writel(HDMI_MC_PHYRSTZ_ASSERT, &hdmi_regs->mc_phyrstz);
	writel(HDMI_MC_HEACPHY_RST_ASSERT, &hdmi_regs->mc_heacphy_rst);

	hdmi_phy_test_clear(1);
	writel(HDMI_PHY_I2CM_SLAVE_ADDR_PHY_GEN2,
		&hdmi_regs->phy_i2cm_slave_addr);
...
static void hdmi_enable_video_path(void)
{
	...
	writel(12, &hdmi_regs->fc_ctrldur);
	writel(32, &hdmi_regs->fc_exctrldur);
	writel(1, &hdmi_regs->fc_exctrlspac);

	/* set to fill tmds data channels */
	writel(0x0b, &hdmi_regs->fc_ch0pream);
	writel(0x16, &hdmi_regs->fc_ch1pream);
	writel(0x21, &hdmi_regs->fc_ch2pream);

	/* enable pixel clock and tmds data path */
	clkdis = 0x7f;
	clkdis &= ~HDMI_MC_CLKDIS_PIXELCLK_DISABLE;
	writel(clkdis, &hdmi_regs->mc_clkdis);
...
static void hdmi_clear_overflow(void)
{
	...
	/* tmds software reset */
	writel((u8)~HDMI_MC_SWRSTZ_TMDSSWRST_REQ, &hdmi_regs->mc_swrstz);

	val = readl(&hdmi_regs->fc_invidconf);

	for (count = 0; count < 4; count++)
		writel(val, &hdmi_regs->fc_invidconf);
}
...
static void hdmi_audio_set_format(void)
{
	...
	writel(0x00, &hdmi_regs->aud_conf2);
}

static void hdmi_audio_fifo_reset(void)
{
	writel((u8)~HDMI_MC_SWRSTZ_II2SSWRST_REQ, &hdmi_regs->mc_swrstz);
	writel(HDMI_AUD_CONF0_SW_AUDIO_FIFO_RST, &hdmi_regs->aud_conf0);

	writel(0x00, &hdmi_regs->aud_int);
	writel(0x00, &hdmi_regs->aud_int1);
}
...
static void hdmi_init_interrupt(void)
{
	 ...
	 */
	ih_mute = readl(&hdmi_regs->ih_mute) |
		  HDMI_IH_MUTE_MUTE_WAKEUP_INTERRUPT |
		  HDMI_IH_MUTE_MUTE_ALL_INTERRUPT;

	writel(ih_mute, &hdmi_regs->ih_mute);

	/* enable i2c master done irq */
	writel(~0x04, &hdmi_regs->i2cm_int);

	/* enable i2c client nack % arbitration error irq */
	writel(~0x44, &hdmi_regs->i2cm_ctlint);

	/* enable phy i2cm done irq */
	writel(HDMI_PHY_I2CM_INT_ADDR_DONE_POL, &hdmi_regs->phy_i2cm_int_addr);

	/* enable phy i2cm nack & arbitration error irq */
	writel(HDMI_PHY_I2CM_CTLINT_ADDR_NAC_POL |
		HDMI_PHY_I2CM_CTLINT_ADDR_ARBITRATION_POL,
		&hdmi_regs->phy_i2cm_ctlint_addr);

	/* enable cable hot plug irq */
	writel((u8)~HDMI_PHY_HPD, &hdmi_regs->phy_mask0);

	/* clear hotplug interrupts */
	writel(HDMI_IH_PHY_STAT0_HPD, &hdmi_regs->ih_phy_stat0);
}

static u8 hdmi_get_plug_in_status(void)
{
	u8 val = readl(&hdmi_regs->phy_stat0) & HDMI_PHY_HPD;
...
static int hdmi_ddc_wait_i2c_done(int msec)
{
	...
	do {
		val = readl(&hdmi_regs->ih_i2cm_stat0);
		if (val & 0x2) {
			writel(val, &hdmi_regs->ih_i2cm_stat0);
			return 0;
		}
...
static int hdmi_read_edid(int block, u8 *buff)
{
	...
	writel(0x7a, &hdmi_regs->i2cm_ss_scl_hcnt_0_addr);
	writel(0x8d, &hdmi_regs->i2cm_ss_scl_lcnt_0_addr);
	clrsetbits_le32(&hdmi_regs->i2cm_div, HDMI_I2CM_DIV_FAST_STD_MODE,
			HDMI_I2CM_DIV_STD_MODE);

	writel(HDMI_I2CM_SLAVE_DDC_ADDR, &hdmi_regs->i2cm_slave);
	writel(HDMI_I2CM_SEGADDR_DDC, &hdmi_regs->i2cm_segaddr);
	...
	while (trytime--) {
		for (n = 0; n < HDMI_EDID_BLOCK_SIZE/8; n++) {
			...
			for (j = 0; j < 8; j++) {
				val = readl(&hdmi_regs->i2cm_buf0 + j);
				buff[8 * n + j] = val;
			}
		}
...
int rk_hdmi_init(u32 vop_id)
{
	...
	writel(RK_SETBITS(1 << 15), &rk3288_grf->soc_con6);
